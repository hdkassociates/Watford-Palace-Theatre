...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<?php watfordpalace_post_thumbnail(); ?>

	<header class="entry-header">
		<?php
			if ( is_single() ) :
				the_title( sprintf( '<h1 class="entry-title"><a href="%s">', esc_url( watfordpalace_get_link_url() ) ), '</a></h1>' );
			else :
				the_title( sprintf( '<h2 class="entry-title"><a href="%s">', esc_url( watfordpalace_get_link_url() ) ), '</a></h2>' );
			endif;
		?>
	</header>
	<!-- .entry-header -->
	...
	<div class="entry-content">
		<?php
			...
			the_content( sprintf(
				__( 'Continue reading %s', 'watfordpalace' ),
